  ...
  const modalElement = createModal(
    `
      ...
      <div>
          ...
          <div class="delete-pet-modal-list-item">Pet Kind: ${petKindsByValue.get(
            pet.kind
          )}</div>
      </div>
  ...
  modalElement
    ...
    .addEventListener('click', async (ev) => {
      if (ev.target.classList.contains('cancel-button')) {
        ...
        const indicatorWrapper = modalElement.querySelector(
          '.indicator-placeholder'
        );

        lockButtons(modalElement);
        indicatorWrapper.replaceChildren(
          createLoadingIndicator('Deleting pet')
        );
        onClose = () => {};
        ...
        try {
          ...
          indicatorWrapper.replaceChildren(
            createErrorIndicator('An error occurred while deleting the pet.')
          );
