            ...
            ${petList
              .map(
                ...
                <tr class="custom-table-row">
                    <th class="custom-table-first-cell" scope="row">${
                      pet.petId
                    }</th>
                    <td class="custom-table-cell">${pet.petName}</td>
                    <td class="custom-table-cell">${formatDate(
                      pet.addedDate
                    )}</td>
                    <td class="custom-table-cell">${petKindsByValue.get(
                      pet.kind
                    )}</td>
...
export const loadPetListPage = async (onDataLoad) => {
  ...
  try {
    ...
    wrapper.replaceChildren(
      createPetsTable(
        petState.petList,
        petState.petKindsByValue,
        petState.petListById
      )
    );
  } catch (error) {
    console.error(error);
